...
function PersonalLoanLiabilities(props) {
    ...
    useEffect(() => {
        retrievePersonalLoanLocalStorage();
    }, [])
    ...
    function addLoan(lstLoanDetails, financialInstitution, loanLimit, amountOwed, repaymentAmount, repaymentType, IsJointAccount) {
        ...
        setPersonalLoanDetails(lstLoanDetails); // Update the state with the new list.
    ...
    function fnSetNewPersonalLoanDetails() {
        //debugger;
        let json = {};
        ...
        if (json.PersonalLoanDetails) {
            let storedData = localStorage.getItem('Personalloan');
            if (storedData) {
                setPersonalLoanDetails(JSON.parse(storedData));
                json.PersonalLoanDetails = JSON.parse(storedData);
            } else {
                json.PersonalLoanDetails = [];
            }
        }
        ...
        const newjsonLoanList = addLoan(json.PersonalLoanDetails, financialInstitution, loanLimit, amountOwed, repaymentAmount, repaymentType, IsJointAccount);

        try {
            localStorage.setItem("Personalloan", JSON.stringify(newjsonLoanList));
            clearStates();
            financialInstitutionref.current.focus();
        } catch (error) {
    ...
    const clearPersonalLoanLocalStorage = (Id, arraylist) => {
        //debugger;
        console.log("clearPersonalLoanLocalStorage Id calls", Id);
        // localStorage.removeItem('Personalloan');
        // alert('Local storage cleared!');
        // setPersonalLoanDetails({});
        let newArray = arraylist.filter((item) => item.id !== Id);
        localStorage.setItem('Personalloan', JSON.stringify(newArray));
        setPersonalLoanDetails(newArray);
    }

    const retrievePersonalLoanLocalStorage = () => {
        let storedData = localStorage.getItem('Personalloan');
        if (storedData) {
            setPersonalLoanDetails(JSON.parse(storedData));
    ...
    const updateloandeatils = (loantype) => (
        ...
        <div id="PersonalloanID" style={{ borderTop: "1px solid var(--shade-color)", borderBottom: "1px solid var(--shade-color)", padding: "10px 10px", backgroundColor: "var(--shade-color)", width: "100%", position: "relative" }}>
                ...
                <div className="input-group" style={{ flexDirection: "column", gap: "10px" }}>
                    <div className="join-dollar">
                        ...
                        <select id="repaymentType"
                            name="repaymentType"
                            value={PersonalloanDetails || 'Monthly'}
            ...
            <div className="controls" >
                ...
                <div className="input-group">
                    <button onClick={() => fnSetNewPersonalLoanDetails()} className="btnAdd">Add</button>
                </div>
            </div>
        </div>
    ...
    const Populateloandeatils = (loantype) => {
        ...
        if (PersonalloanDetails.length === 0) return null;

        return PersonalloanDetails.map((item, index, array) => (
            <div key={index} style={{ marginBottom: "10px", borderTop: "1px solid var(--shade-color)", borderBottom: "1px solid var(--shade-color)", padding: "10px 10px", backgroundColor: "var(--shade-color)", width: "100%", position: "relative" }}>
                <CloseIcon style={{ position: "absolute", color: "red", fontSize: "18px", right: "5", top: "5" }} onClick={() => clearPersonalLoanLocalStorage(item.id, array)} />
                <div className="controls" style={{ marginTop: "20px" }}>
                    <div className="input-group-label">
                        <p className="loans-heads">{loantype}</p>
                    </div>
    ...
    return (<>
        {
            Populateloandeatils('Personal loan')
        }

        {
            updateloandeatils('Personal loan')
        }
    </>)
}

export default PersonalLoanLiabilities;
