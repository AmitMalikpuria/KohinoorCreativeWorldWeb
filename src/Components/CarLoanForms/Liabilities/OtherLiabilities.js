...
function OtherLiabilities(props) {
    ...
    const [OtherLoanLiabilities, setOtherLoanLiabilities] = useState([]);

    useEffect(() => {
        retrieveOtherLoanLocalStorage();
    }, [])
    ...
    function addLoan(lstLoanDetails, financialInstitution, loanLimit, amountOwed, repaymentAmount, repaymentType, IsJointAccount) {
        ...
        setOtherLoanLiabilities(lstLoanDetails); // Update the state with the new list.
    ...
    function fnSetNewOtherLoanLiabilities() {
        //debugger;
        let json = {};
        ...
        if (json.OtherLoanDetails) {
            let storedData = localStorage.getItem('OtherLoan');
            if (storedData) {
                setOtherLoanLiabilities(JSON.parse(storedData));
                json.OtherLoanDetails = JSON.parse(storedData);
            } else {
                json.OtherLoanDetails = [];
            }
        }
        ...
        const newjsonLoanList = addLoan(json.OtherLoanDetails, financialInstitution, loanLimit, amountOwed, repaymentAmount, repaymentType, IsJointAccount);

        try {
            localStorage.setItem("OtherLoan", JSON.stringify(newjsonLoanList));
            clearStates();
            financialInstitutionref.current.focus();
            //navigate("/secured-car-loan#OtherLoanID");
        } catch (error) {
    ...
    const clearOtherLoanLocalStorage = (Id, arraylist) => {
        //debugger;
        console.log("clearOtherLoanLocalStorage Id calls", Id);
        // localStorage.removeItem('OtherLoan');
        // alert('Local storage cleared!');
        // setOtherLoanLiabilities({});
        let newArray = arraylist.filter((item) => item.id !== Id);
        localStorage.setItem('OtherLoan', JSON.stringify(newArray));
        setOtherLoanLiabilities(newArray);
    }

    const retrieveOtherLoanLocalStorage = () => {
        let storedData = localStorage.getItem('OtherLoan');
        if (storedData) {
            setOtherLoanLiabilities(JSON.parse(storedData));
    ...
    const updateloandeatils = (loantype) => (
        ...
        <div id="OtherLoanID" style={{ borderTop: "1px solid var(--shade-color)", borderBottom: "1px solid var(--shade-color)", padding: "10px 10px", backgroundColor: "var(--shade-color)", width: "100%", position: "relative" }}>
                ...
                <div className="input-group" style={{ flexDirection: "column", gap: "10px" }}>
                    <div className="join-dollar">
                        ...
                        <select id="repaymentType"
                            name="repaymentType"
                            value={OtherLoanLiabilities || 'Monthly'}
            ...
            <div className="controls" >
                ...
                <div className="input-group">
                    <button onClick={() => fnSetNewOtherLoanLiabilities()} className="btnAdd">Add</button>
                </div>
            </div>
        </div>
    ...
    const Populateloandeatils = (loantype) => {
        ...
        if (OtherLoanLiabilities.length === 0) return null;

        return OtherLoanLiabilities.map((item, index, array) => (
            <div key={index} style={{ marginBottom: "10px", borderTop: "1px solid var(--shade-color)", borderBottom: "1px solid var(--shade-color)", padding: "10px 10px", backgroundColor: "var(--shade-color)", width: "100%", position: "relative" }}>
                <CloseIcon style={{ position: "absolute", color: "red", fontSize: "18px", right: "5", top: "5" }} onClick={() => clearOtherLoanLocalStorage(item.id, array)} />
                <div className="controls" style={{ marginTop: "20px" }}>
                    <div className="input-group-label">
                        <p className="loans-heads">{loantype}</p>
                    </div>
    ...
    return (<>
        {
            Populateloandeatils('Liability')
        }

        {
            updateloandeatils('Liability')
        }
    </>)
}
