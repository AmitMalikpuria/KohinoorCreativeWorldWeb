...
function CreditCardLoanLiabilities(props) {
    ...
    const [CreditCardLoanDetails, setCreditCardLoanDetails] = useState([]);

    useEffect(() => {
        retrieveCreditCardLoanLocalStorage();
    }, [])
    ...
    function addLoan(lstLoanDetails, financialInstitution, loanLimit, amountOwed, repaymentAmount, repaymentType, IsJointAccount) {
        ...
        setCreditCardLoanDetails(lstLoanDetails); // Update the state with the new list.
    ...
    function fnSetNewCreditCardLoanDetails() {
        //debugger;
        let json = {};
        ...
        if (json.CreditCardLoanDetails) {
            let storedData = localStorage.getItem('CreditCardloan');
            if (storedData) {
                setCreditCardLoanDetails(JSON.parse(storedData));
                json.CreditCardLoanDetails = JSON.parse(storedData);
            } else {
                json.CreditCardLoanDetails = [];
            }
        }
        ...
        const newjsonLoanList = addLoan(json.CreditCardLoanDetails, financialInstitution, loanLimit, amountOwed, repaymentAmount, repaymentType, IsJointAccount);

        try {
            localStorage.setItem("CreditCardloan", JSON.stringify(newjsonLoanList));
            clearStates();
            financialInstitutionref.current.focus();
            //navigate("/secured-car-loan#CreditCardloanID");
        } catch (error) {
    ...
    const clearCreditCardloanLocalStorage = (Id, arraylist) => {
        //debugger;
        console.log("clearCreditCardloanLocalStorage Id calls", Id);
        // localStorage.removeItem('CreditCardloan');
        // alert('Local storage cleared!');
        // setCreditCardLoanDetails({});
        let newArray = arraylist.filter((item) => item.id !== Id);
        localStorage.setItem('CreditCardloan', JSON.stringify(newArray));
        setCreditCardLoanDetails(newArray);
    }

    const retrieveCreditCardLoanLocalStorage = () => {
        let storedData = localStorage.getItem('CreditCardloan');
        if (storedData) {
            setCreditCardLoanDetails(JSON.parse(storedData));
        }
    ...
    const updateloandeatils = (loantype) => (
        ...
        <div id="CreditCardloanID" style={{ borderTop: "1px solid var(--shade-color)", borderBottom: "1px solid var(--shade-color)", padding: "10px 10px", backgroundColor: "var(--shade-color)", width: "100%", position: "relative" }}>
                ...
                <div className="input-group" style={{ flexDirection: "column", gap: "10px" }}>
                    <div className="join-dollar">
                        ...
                        <select id="repaymentType"
                            name="repaymentType"
                            value={CreditCardLoanDetails || 'Monthly'}
            ...
            <div className="controls" >
                ...
                <div className="input-group">
                    <button onClick={() => fnSetNewCreditCardLoanDetails()} className="btnAdd">Add</button>
                </div>
            </div>
        </div>
    ...
    const Populateloandeatils = (loantype) => {
        ...
        if (CreditCardLoanDetails.length === 0) return null;

        return CreditCardLoanDetails.map((item, index, array) => (
            <div key={index} style={{ marginBottom: "10px", borderTop: "1px solid var(--shade-color)", borderBottom: "1px solid var(--shade-color)", padding: "10px 10px", backgroundColor: "var(--shade-color)", width: "100%", position: "relative" }}>
                <CloseIcon style={{ position: "absolute", color: "red", fontSize: "18px", right: "5", top: "5" }} onClick={() => clearCreditCardloanLocalStorage(item.id, array)} />
                <div className="controls" style={{ marginTop: "20px" }}>
                    <div className="input-group-label">
                        <p className="loans-heads">{loantype}</p>
                    </div>
    ...
    return (<>
        {
            Populateloandeatils('Credit/Store cards')
        }

        {
            updateloandeatils('Credit/Store cards')
        }
    </>)
}

export default CreditCardLoanLiabilities;
